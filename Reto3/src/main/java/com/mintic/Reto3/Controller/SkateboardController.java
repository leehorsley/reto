...
import com.mintic.Reto3.Service.SkateboardService;
...
public class SkateboardController {
    ...
    @GetMapping("/{id}")
    public Optional<Skate> getSkateboard(@PathVariable("id") int id) {
        return skateboardService.getSkate(id);
    }
    ...
    public Skate update(@RequestBody Skate skate) {
        return skateboardService.update(skate);
    }

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public boolean delete(@PathVariable("id") int id) {
        return skateboardService.delete(id);
    }
